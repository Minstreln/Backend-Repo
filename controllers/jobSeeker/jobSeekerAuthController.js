...
exports.jobSeekerSignup = catchAsync(async (req, res) => {
  const existingRecruiter = await Recruiter.findOne({ email: req.body.email });

  if (existingRecruiter) {
      return res.status(400).json({
          status: 'fail',
          message: 'This email is already registered as a recruiter. You cannot sign up with same email credentials as a jobseeker.'
      });
  }
